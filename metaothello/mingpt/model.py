"""
Code adapted from Karpathy's minGPT.
"""
import math
import logging
...
class GPTConfig:
    ...
    def __init__(self, vocab_size, block_size, **kwargs):
        ...
        for k,v in kwargs.items():
            setattr(self, k, v)
        logger.info("model config: %s" % str(self.__dict__))
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=0)  # ignore_index=0 matches PAD_TOKEN ("[PAD]") at vocab index 0
        return logits, loss
